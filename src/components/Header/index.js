...
const ResponsiveAppBar = () => {
  ...
  return (
    <AppBar style={{background:'#736673'}} position="sticky">
      <Container maxWidth="xl">
        <Toolbar disableGutters>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }}>
            ...
            >
              {pages.map((page) => (
                <Link key={page} to={`/${page}`} style={{textDecoration: 'none', color:"#072040"}}>
                <MenuItem key={page}>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }} style={{justifyContent: 'flex-end'}}>
            {pages.map((page) => (
              <Link key={page} to={`/${page}`} style={{textDecoration: "none", color:"#fff"}}>
              <Button
                key={page}
